...
plot(Uni_pcoa$vectors[,1:2], bg= c("skyblue","palegreen", "orangered")[as.factor(as.data.frame(samples)$Status)], pch=21, cex=2, xlab=paste0("PCoA", pc[1], " (", mds.var.per[1], "%)"), ylab=paste0("PCoA", pc[2], " (", mds.var.per[2], "%)"))
ordiellipse(Uni_pcoa$vectors[,1:2], as.data.frame(samples)$Status, kind="sd", lwd=1, lty=3, draw = "polygon", alpha = 50, col = c("skyblue","palegreen", "orangered"))
