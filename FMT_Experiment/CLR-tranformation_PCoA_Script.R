...
jpeg("Eucl_PCoA.jpg", height = 10, width = 10, units = 'in', res = 600)
plot(eucl_pcoa$vectors[,1:2], bg= c("darkolivegreen4", "red", "darkgreen", "salmon4")[as.factor(as.data.frame(samples)$Status)], pch=21, cex=2, xlab=paste0("PCoA", pc[1], " (", mds.var.per[1], "%)"), ylab=paste0("PCoA", pc[2], " (", mds.var.per[2], "%)"))
#text(eucl_pcoa$vectors[,1:2], labels=rownames(Bushman2@sam_data), cex=0.3, font=1, pos=1)
ordiellipse(eucl_pcoa$vectors[,1:2], as.data.frame(samples)$Status, kind="sd", lwd=1, lty=3, draw = "polygon", alpha = 50, col = c("darkolivegreen4", "red", "darkgreen", "salmon4"))
#ordispider(eucl_pcoa$vectors[,1:2], as.data.frame(samples)$Status, lty=3, spider ="centroid", lwd=1, col="black")
